...
class StateManagerTest {
    ...
    @Test
    fun `Process delivers action to all middlewares`() = runTest {
        ...
        val stateManager = createStateManager(processor, this, middleware1, middleware2)
    ...
    @Test
    fun `Process delivers action to added middlewares`() = runTest {
        val middleware1: IMiddleware<String> = mockk(relaxed = true)
        val middleware2: IMiddleware<String> = mockk(relaxed = true)
        val processor: IReducer<String> = mockk(relaxed = true)
        val stateManager = createStateManager(processor, this, middleware1)
    ...
    @Test
    fun `Reducing an ISideEffect action makes it process the side-effect`() = runTest {
        val reducer: IReducer<String> = mockk(relaxed = true)
        val middleware1: IMiddleware<String> = mockk(relaxed = true)
        every { reducer.reduce(any(), any()) } returns "new state"
        val stateManager = spyk(createStateManager(reducer, this, middleware1))
    ...
    fun `Using async middleware`() = runTest {
        val middleware1: IMiddleware<String> = spyk(
            object : IMiddleware<String> {
                override suspend fun process(action: IAction, state: String, processor: IProcessor<String>) = Unit
            })
        val reducer: IReducer<String> = mockk(relaxed = true)
        val stateManager = spyk(createStateManager(reducer, this, middleware1))
        ...
        stateManager.perform(Action)
        advanceUntilIdle()
        coVerify { middleware1.process(any(), any(), any()) }
    }
